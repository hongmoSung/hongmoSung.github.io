...
// After: 어댑터를 통한 결합 감소
class AudioPlayer {
    MediaPlayerAdapter mediaPlayerAdapter;  // VlcPlayer, Mp4Player 를 전혀 모름 (생성은 어댑터가 담당)
}
```

> 위 예제의 `AudioPlayer`는 `new MediaPlayerAdapter(...)`로 어댑터를 직접 생성하므로, Adaptee에 대한 의존은 사라졌지만 어댑터 구현 클래스에는 여전히 의존한다.
> 인터페이스에만 의존하게 하려면 `MediaPlayer` 타입의 어댑터를 생성자로 주입받으면 된다.

#### **응집도 향상 (Cohesion Improvement)**

**책임 분리:**
- `AudioPlayer`: 오직 음악 재생만
- `MediaPlayerAdapter`: 오직 인터페이스 변환만
