...
graph TB
    ...
    %% 메모리 적재
    READ -->|클래스 메타데이터| MA
    CREATE -->|java.lang.Class 객체| HEAP
    CLINIT --> HEAP
...
- **Class 객체 생성**: 읽어들인 클래스의 메타데이터를 메서드 영역(Metaspace)에 저장하고, 이를 가리키는 `java.lang.Class` 객체를 힙에 생성한다.
...
필드 정보(이름, 데이터 타입, 접근 제어자), 메소드 정보(이름, 리턴 타입, 파라미터, 접근 제어자), 타입 정보, 런타임 상수 풀(Constant Pool) 등 클래스 레벨의 메타데이터를 저장하는 영역이다. (HotSpot JDK 7 부터 static 변수의 값은 메서드 영역이 아니라 힙의 `Class` 객체 안에 저장된다.) 이 영역은 JVM이 시작될 때 생성되며, JVM 내의 모든 스레드가 공유하는 단일 영역이다.
...
graph TB
    ...
    subgraph "Heap Memory"
        subgraph "Young Generation"
            ...
            S0["Survivor 0<br/>(from/to 교대)"]
            S1["Survivor 1<br/>(from/to 교대)"]
        end
    ...
    %% 객체 이동
    EDEN -->|생존 객체| S0
    S0 <-->|Minor GC마다 생존 객체를 반대편으로 복사| S1
    S1 -->|나이 임계값 도달| OLD
...
- **Young Generation (젊은 세대)**: 새로 생성된 대부분의 객체가 할당되는 공간이다. Eden Space와 두 개의 Survivor Spaces(S0, S1)로 구성된다. 새 객체는 주로 Eden Space에 할당되며, Eden Space가 가득 차면 Minor GC(Young GC)가 발생한다. Minor GC 후 살아남은 객체들은 Survivor Space(S0 또는 S1)로 이동하며, 이 과정을 반복하면서 객체의 "나이(age)"가 증가한다. S0와 S1은 나이로 구분되는 공간이 아니다. 한쪽은 항상 비어 있고, Minor GC마다 Eden과 사용 중인 Survivor(from)의 생존 객체를 빈 Survivor(to)로 복사한 뒤 두 공간의 역할을 맞바꾼다. 나이는 객체 헤더에 기록된다. 일정 나이 임계값(Tenuring Threshold)에 도달하면 Old Generation으로 승격된다.
...
|----------------------------------------|---------------------------------------|-----------|-----------------------------------------------------------|
| **메서드 영역 (Method Area / Metaspace)**   | 클래스 메타데이터 저장                         | 공유 (단일)   | 클래스/인터페이스 정의, 필드/메소드 정보, 런타임 상수 풀 (static 변수 값은 힙의 Class 객체) |
