...
- **프론트엔드 배포**: React(Vite) 빌드 결과물(dist), Next.js 정적 export 결과물(out)을 nginx로 서빙하기
...
npm install
npm run build
# React(Vite): dist 디렉토리 생성 (CRA 는 build)
# Next.js: next.config 에 output: 'export' 를 설정한 경우 out 디렉토리 생성 (기본은 .next)
```
